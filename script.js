...
class TravelersApp {
  ...
  renderGallery() {
    ...
    gallery.innerHTML = sortedAnimals
      .map((animal) => this.createCharacterCard(animal))
      .join("");
  ...
  createCharacterCard(animal) {
    ...
    const isLive = animal.status === "live" || unlockDate <= today;
    const formattedUnlockDate = this.formatDate(animal.unlockDate);

    return `
            <article class="character-card ${isLive ? "live" : "locked"}" 
      ...
      isLive ? "Available now" : "Coming soon"
                ...
                ${!isLive ? this.createLockOverlay(formattedUnlockDate) : ""}
  ...
  renderTimeline() {
    ...
    timeline.innerHTML = sortedAnimals
      .map((animal) => {
        const unlockDate = new Date(animal.unlockDate);
        const weekNumber = this.getWeekNumber(unlockDate);
        const isLive = animal.status === "live" || unlockDate <= today;
        const isCurrent =
          this.isToday(animal.unlockDate) ||
          (unlockDate <= today && animal.status !== "live");
        ...
        if (isLive) {
          ...
          statusText = `<span class="timeline-status">Unlocks ${this.formatDate(
            animal.unlockDate
          )}</span>`;
        }
        ...
        return `
          ...
          isLive ? animal.name : "Mystery character"
        }">
